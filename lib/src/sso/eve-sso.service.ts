...
export class EveSsoService {
   ...
   * sometimes with `InvalidRefreshTokenException` in normal use. You should
   * handle at least that error type properly.
   */
   ...
   */
  async verifyAndDecodeSsoAccessToken(accessToken: string): Promise<EveSsoVerifyTokenResponse> {
    try {
   ...
   *
   * EVE SSO API will respond with 200 regardless of whether or not the token
   * was actually revoked or not found.
   */
